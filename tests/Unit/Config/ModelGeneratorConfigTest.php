...
final class ModelGeneratorConfigTest extends TestCase
{
    ...
    #[Test]
    public function it_merges_config_arrays(): void
    {
        // Arrange
        $_ENV['EMG_MODEL_TRAITS'] = 'App\\Traits\\HasUuid';
        $_ENV['EMG_ADDITIONAL_TRAITS'] = 'App\\Traits\\Searchable';

        // Act
        $config = ModelGeneratorConfig::fromEnvironment();

        // Assert
        expect($config->get('traits'))->toBe([
            'App\\Traits\\HasUuid',
    ...
    #[Test]
    public function it_handles_nested_configuration(): void
    {
        // Arrange
        $_ENV['EMG_QUEUE_CONNECTION'] = 'redis';
        $_ENV['EMG_QUEUE_RETRIES'] = '3';
        $_ENV['EMG_QUEUE_RETRY_DELAY'] = '5';

        // Act
        $config = ModelGeneratorConfig::fromEnvironment();

        // Assert
        expect($config->get('queue.connection'))->toBe('redis')
            ->and($config->get('queue.retries'))->toBe(3)
            ->and($config->get('queue.retry_delay'))->toBe(5);
    }
}
